...
export class SlotListClockComponentsPage {
  createButton = element(by.css('ion-fab-button'));
  viewButtons = element.all(by.css('ion-item'));
  title = element.all(by.css('ion-title')).get(2);
  noResult = element(by.cssContainingText('ion-label', 'No Slot List Clocks found.'));
  entities = element.all(by.css('page-slot-list-clock ion-item'));
...
export class SlotListClockUpdatePage {
...
export class SlotListClockDetailPage {
  pageTitle = element.all(by.css('ion-title')).get(3);
