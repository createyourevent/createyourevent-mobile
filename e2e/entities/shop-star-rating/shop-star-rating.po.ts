...
export class ShopStarRatingComponentsPage {
  createButton = element(by.css('ion-fab-button'));
  viewButtons = element.all(by.css('ion-item'));
  title = element.all(by.css('ion-title')).get(2);
  noResult = element(by.cssContainingText('ion-label', 'No Shop Star Ratings found.'));
  entities = element.all(by.css('page-shop-star-rating ion-item'));
...
export class ShopStarRatingUpdatePage {
...
export class ShopStarRatingDetailPage {
  pageTitle = element.all(by.css('ion-title')).get(3);
