...
export class GiftShoppingCartComponentsPage {
  createButton = element(by.css('ion-fab-button'));
  viewButtons = element.all(by.css('ion-item'));
  title = element.all(by.css('ion-title')).get(2);
  noResult = element(by.cssContainingText('ion-label', 'No Gift Shopping Carts found.'));
  entities = element.all(by.css('page-gift-shopping-cart ion-item'));
...
export class GiftShoppingCartUpdatePage {
...
export class GiftShoppingCartDetailPage {
  pageTitle = element.all(by.css('ion-title')).get(3);
