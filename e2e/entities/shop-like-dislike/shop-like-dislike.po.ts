...
export class ShopLikeDislikeComponentsPage {
  createButton = element(by.css('ion-fab-button'));
  viewButtons = element.all(by.css('ion-item'));
  title = element.all(by.css('ion-title')).get(2);
  noResult = element(by.cssContainingText('ion-label', 'No Shop Like Dislikes found.'));
  entities = element.all(by.css('page-shop-like-dislike ion-item'));
...
export class ShopLikeDislikeUpdatePage {
...
export class ShopLikeDislikeDetailPage {
  pageTitle = element.all(by.css('ion-title')).get(3);
