...
export class EventCommentComponentsPage {
  createButton = element(by.css('ion-fab-button'));
  viewButtons = element.all(by.css('ion-item'));
  title = element.all(by.css('ion-title')).get(2);
  noResult = element(by.cssContainingText('ion-label', 'No Event Comments found.'));
  entities = element.all(by.css('page-event-comment ion-item'));
...
export class EventCommentUpdatePage {
...
export class EventCommentDetailPage {
  pageTitle = element.all(by.css('ion-title')).get(3);
