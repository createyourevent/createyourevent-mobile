...
export class OrganizationStarRatingComponentsPage {
  createButton = element(by.css('ion-fab-button'));
  viewButtons = element.all(by.css('ion-item'));
  title = element.all(by.css('ion-title')).get(2);
  noResult = element(by.cssContainingText('ion-label', 'No Organization Star Ratings found.'));
  entities = element.all(by.css('page-organization-star-rating ion-item'));
...
export class OrganizationStarRatingUpdatePage {
...
export class OrganizationStarRatingDetailPage {
  pageTitle = element.all(by.css('ion-title')).get(3);
