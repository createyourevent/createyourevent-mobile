...
export class EventStarRatingComponentsPage {
  createButton = element(by.css('ion-fab-button'));
  viewButtons = element.all(by.css('ion-item'));
  title = element.all(by.css('ion-title')).get(2);
  noResult = element(by.cssContainingText('ion-label', 'No Event Star Ratings found.'));
  entities = element.all(by.css('page-event-star-rating ion-item'));
...
export class EventStarRatingUpdatePage {
...
export class EventStarRatingDetailPage {
  pageTitle = element.all(by.css('ion-title')).get(3);
