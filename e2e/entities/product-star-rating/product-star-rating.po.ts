...
export class ProductStarRatingComponentsPage {
  createButton = element(by.css('ion-fab-button'));
  viewButtons = element.all(by.css('ion-item'));
  title = element.all(by.css('ion-title')).get(2);
  noResult = element(by.cssContainingText('ion-label', 'No Product Star Ratings found.'));
  entities = element.all(by.css('page-product-star-rating ion-item'));
...
export class ProductStarRatingUpdatePage {
...
export class ProductStarRatingDetailPage {
  pageTitle = element.all(by.css('ion-title')).get(3);
