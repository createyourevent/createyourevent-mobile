...
export class EventLikeDislikeComponentsPage {
  createButton = element(by.css('ion-fab-button'));
  viewButtons = element.all(by.css('ion-item'));
  title = element.all(by.css('ion-title')).get(2);
  noResult = element(by.cssContainingText('ion-label', 'No Event Like Dislikes found.'));
  entities = element.all(by.css('page-event-like-dislike ion-item'));
...
export class EventLikeDislikeUpdatePage {
...
export class EventLikeDislikeDetailPage {
  pageTitle = element.all(by.css('ion-title')).get(3);
