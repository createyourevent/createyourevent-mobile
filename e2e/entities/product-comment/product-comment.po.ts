...
export class ProductCommentComponentsPage {
  createButton = element(by.css('ion-fab-button'));
  viewButtons = element.all(by.css('ion-item'));
  title = element.all(by.css('ion-title')).get(2);
  noResult = element(by.cssContainingText('ion-label', 'No Product Comments found.'));
  entities = element.all(by.css('page-product-comment ion-item'));
...
export class ProductCommentUpdatePage {
...
export class ProductCommentDetailPage {
  pageTitle = element.all(by.css('ion-title')).get(3);
