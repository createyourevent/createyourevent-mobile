...
export class SlotListPlumComponentsPage {
  createButton = element(by.css('ion-fab-button'));
  viewButtons = element.all(by.css('ion-item'));
  title = element.all(by.css('ion-title')).get(2);
  noResult = element(by.cssContainingText('ion-label', 'No Slot List Plums found.'));
  entities = element.all(by.css('page-slot-list-plum ion-item'));
...
export class SlotListPlumUpdatePage {
...
export class SlotListPlumDetailPage {
  pageTitle = element.all(by.css('ion-title')).get(3);
