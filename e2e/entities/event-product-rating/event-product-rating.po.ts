...
export class EventProductRatingComponentsPage {
  createButton = element(by.css('ion-fab-button'));
  viewButtons = element.all(by.css('ion-item'));
  title = element.all(by.css('ion-title')).get(2);
  noResult = element(by.cssContainingText('ion-label', 'No Event Product Ratings found.'));
  entities = element.all(by.css('page-event-product-rating ion-item'));
...
export class EventProductRatingUpdatePage {
...
export class EventProductRatingDetailPage {
  pageTitle = element.all(by.css('ion-title')).get(3);
