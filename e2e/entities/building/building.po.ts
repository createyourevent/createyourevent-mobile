...
export class BuildingComponentsPage {
  ...
  noResult = element(by.cssContainingText('ion-label', 'No Buildings found.'));
  entities = element.all(by.css('page-building ion-item'));
...
export class BuildingUpdatePage {
  ...
  surfaceInput = element(by.css('ion-input[formControlName="surface"] input'));
...
export class BuildingDetailPage {
  ...
  surfaceInput = element.all(by.css('span')).get(2);
  ...
  async getSurfaceInput(): Promise<string> {
    return await this.surfaceInput.getText();
  }
