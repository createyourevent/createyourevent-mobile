...
export class SlotListCherryComponentsPage {
  createButton = element(by.css('ion-fab-button'));
  viewButtons = element.all(by.css('ion-item'));
  title = element.all(by.css('ion-title')).get(2);
  noResult = element(by.cssContainingText('ion-label', 'No Slot List Cherries found.'));
  entities = element.all(by.css('page-slot-list-cherry ion-item'));
...
export class SlotListCherryUpdatePage {
...
export class SlotListCherryDetailPage {
  pageTitle = element.all(by.css('ion-title')).get(3);
