...
export class FeeBalanceComponentsPage {
  createButton = element(by.css('ion-fab-button'));
  viewButtons = element.all(by.css('ion-item'));
  title = element.all(by.css('ion-title')).get(2);
  noResult = element(by.cssContainingText('ion-label', 'No Fee Balances found.'));
  entities = element.all(by.css('page-fee-balance ion-item'));
...
export class FeeBalanceUpdatePage {
...
export class FeeBalanceDetailPage {
  pageTitle = element.all(by.css('ion-title')).get(3);
  deleteButton = element(by.css('ion-button[color="danger"]'));
  idInput = element.all(by.css('span')).get(1);

  totalInput = element.all(by.css('span')).get(4);
