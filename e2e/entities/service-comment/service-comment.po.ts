...
export class ServiceCommentComponentsPage {
  createButton = element(by.css('ion-fab-button'));
  viewButtons = element.all(by.css('ion-item'));
  title = element.all(by.css('ion-title')).get(2);
  noResult = element(by.cssContainingText('ion-label', 'No Service Comments found.'));
  entities = element.all(by.css('page-service-comment ion-item'));
...
export class ServiceCommentUpdatePage {
...
export class ServiceCommentDetailPage {
  pageTitle = element.all(by.css('ion-title')).get(3);
