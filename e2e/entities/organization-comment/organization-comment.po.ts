...
export class OrganizationCommentComponentsPage {
  createButton = element(by.css('ion-fab-button'));
  viewButtons = element.all(by.css('ion-item'));
  title = element.all(by.css('ion-title')).get(2);
  noResult = element(by.cssContainingText('ion-label', 'No Organization Comments found.'));
  entities = element.all(by.css('page-organization-comment ion-item'));
...
export class OrganizationCommentUpdatePage {
...
export class OrganizationCommentDetailPage {
  pageTitle = element.all(by.css('ion-title')).get(3);
