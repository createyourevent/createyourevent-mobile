...
export class ShopCommentComponentsPage {
  createButton = element(by.css('ion-fab-button'));
  viewButtons = element.all(by.css('ion-item'));
  title = element.all(by.css('ion-title')).get(2);
  noResult = element(by.cssContainingText('ion-label', 'No Shop Comments found.'));
  entities = element.all(by.css('page-shop-comment ion-item'));
...
export class ShopCommentUpdatePage {
...
export class ShopCommentDetailPage {
  pageTitle = element.all(by.css('ion-title')).get(3);
