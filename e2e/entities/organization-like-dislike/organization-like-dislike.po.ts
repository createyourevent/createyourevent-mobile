...
export class OrganizationLikeDislikeComponentsPage {
  createButton = element(by.css('ion-fab-button'));
  viewButtons = element.all(by.css('ion-item'));
  title = element.all(by.css('ion-title')).get(2);
  noResult = element(by.cssContainingText('ion-label', 'No Organization Like Dislikes found.'));
  entities = element.all(by.css('page-organization-like-dislike ion-item'));
...
export class OrganizationLikeDislikeUpdatePage {
...
export class OrganizationLikeDislikeDetailPage {
  pageTitle = element.all(by.css('ion-title')).get(3);
