...
export class ProductLikeDislikeComponentsPage {
  createButton = element(by.css('ion-fab-button'));
  viewButtons = element.all(by.css('ion-item'));
  title = element.all(by.css('ion-title')).get(2);
  noResult = element(by.cssContainingText('ion-label', 'No Product Like Dislikes found.'));
  entities = element.all(by.css('page-product-like-dislike ion-item'));
...
export class ProductLikeDislikeUpdatePage {
...
export class ProductLikeDislikeDetailPage {
  pageTitle = element.all(by.css('ion-title')).get(3);
